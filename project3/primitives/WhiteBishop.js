class WhiteBishop extends Bishop {
    ...
    */
    ...
    dead(deadId){
        let controlPoints = [];
            controlPoints[0] = vec3.fromValues(5.5 + Math.floor(deadId / 13) + this.row,-1.4,1.5 - (deadId % 13) + this.column);
            controlPoints[1] = vec3.fromValues(5.5 + Math.floor(deadId / 13) + this.row,3.5,1.5 - (deadId % 13) + this.column);
